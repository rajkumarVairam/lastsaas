...
This guide details how to run the SaaSQuickStart application locally, especially optimized for Windows, without disturbing the default `README.md` and keeping upstream merges clean. You can add `LOCAL_SETUP.md` to your `.gitignore` if you do not wish to push this file.
...
When you test the checkout flow locally, SaaSQuickStart's database will not automatically upgrade your account. This is because Stripe (on the public internet) cannot send success Webhooks to your local laptop (`localhost`). 
...
# Then, forward webhooks directly to the SaaSQuickStart backend:
...
This approach uses SaaSQuickStart's native multi-tenancy. You only run one instance of the application, and you manage the client as an organization inside your admin dashboard.
...
To save you from debugging, here are two major architectural design choices the SaaSQuickStart team made that you must know:
...
fly secrets set `
  ...
  DATABASE_NAME="saasquickstart-prod" `
...
```powershell
fly ssh console -C "/app/saasquickstart setup"
```
...
- **Stripe Plans:** You can completely create and manage Stripe subscription packages (e.g., Starter, Pro, Enterprise) directly from **Admin → Plans**. You define the pricing, limits, entitlements, and trial days here; SaaSQuickStart will automatically sync this to Stripe. No need to touch the Stripe dashboard!
