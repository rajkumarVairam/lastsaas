...
func apiReference() []apiSection {
 ...
 "memberships":[{"tenantId":"...","tenantName":"Jane's Team","tenantSlug":"janes-team","role":"owner","isRoot":false}]}`,
				...
				},
				{
					...
					Detail:   "Updates the authenticated user's preferences. All fields are optional — only provided fields are changed. <code>emailMarketing</code> controls whether the user receives marketing emails (newsletters, product updates). Transactional emails (security, billing) are always sent regardless of this setting.",
					Auth:     "jwt",
					Body:     `{"themePreference":"dark","emailMarketing":false}`,
				...
				},
				{
					...
					Detail:   "Sets <code>emailPreferences.marketing</code> to false for the user identified by the token. No authentication required — the token in the URL is sufficient. Returns a plain HTML confirmation page suitable for email clients.",
					Auth:     "none",
					Params:   []apiParam{{"token", "string", true, "Unsubscribe token from the email footer link"}},
			...
			Endpoints: []apiEndpoint{
				{
					...
					Summary:  "List jobs",
					Detail:   "Returns a paginated list of background jobs for the tenant. Filter by <code>status</code> (pending, running, completed, failed, dead, cancelled) and <code>type</code>.",
					Auth:     "jwt+tenant",
					Params: []apiParam{
						{Name: "status", Type: "string", Required: false, Desc: "Filter by status: pending, running, completed, failed, dead, cancelled"},
						{Name: "type", Type: "string", Required: false, Desc: "Filter by job type (e.g. social_post.publish)"},
						{Name: "page", Type: "int", Required: false, Desc: "Page number (default 1)"},
					},
					Response: `{"jobs":[{"id":"...","type":"social_post.publish","status":"pending","runAt":"2024-01-01T00:00:00Z","attempts":0,"maxAttempts":3,"createdAt":"2024-01-01T00:00:00Z","updatedAt":"2024-01-01T00:00:00Z"}],"total":1}`,
				},
				{
					Method:  "POST",
					Path:    "/api/tenant/jobs",
					Summary: "Enqueue a job",
					Detail:  "Schedules a background job for the tenant. The <code>type</code> must match a registered handler. Omit <code>runAt</code> to run immediately.",
					Auth:    "jwt+tenant",
					Body:    `{"type":"social_post.publish","payload":{"postId":"..."},"runAt":"2024-01-01T00:00:00Z","maxAttempts":3}`,
					Response: `{"id":"...","type":"social_post.publish","status":"pending","runAt":"2024-01-01T00:00:00Z"}`,
				},
				{
					Method:   "GET",
					Path:     "/api/tenant/jobs/{jobId}",
					Summary:  "Get job",
					Detail:   "Returns a single job by ID, including its result and last error if applicable.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "jobId", Type: "objectId", Required: true, Desc: "Job ID"}},
					Response: `{"id":"...","type":"social_post.publish","status":"completed","result":{"publishedAt":"2024-01-01T00:00:00Z"}}`,
				},
				{
					Method:   "DELETE",
					Path:     "/api/tenant/jobs/{jobId}",
					Summary:  "Cancel job",
					Detail:   "Cancels a pending, failed, or dead job. Running jobs cannot be cancelled.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "jobId", Type: "objectId", Required: true, Desc: "Job ID"}},
					Response: `{"status":"ok"}`,
				},
				{
					Method:   "POST",
					Path:     "/api/tenant/jobs/{jobId}/retry",
					Summary:  "Retry job",
					Detail:   "Re-queues a dead, failed, or cancelled job for immediate execution.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "jobId", Type: "objectId", Required: true, Desc: "Job ID"}},
					Response: `{"status":"ok"}`,
				},
			...
			Endpoints: []apiEndpoint{
				{
					...
					Detail:   "Returns tenant documents visible to the caller. Tenant-scoped: members see all <code>tenant</code>-visibility docs plus their own <code>owner</code>-only docs. Add <code>?mine=true</code> to restrict to own uploads only.",
					Auth:     "jwt+tenant",
					Params: []apiParam{
						{Name: "mine", Type: "string", Required: false, Desc: "Set to 'true' to return only your own uploads"},
						{Name: "page", Type: "int", Required: false, Desc: "Page number (default 1, page size 50)"},
					},
					Response: `{"documents":[{"id":"...","tenantId":"...","ownerId":"...","filename":"tax_return.pdf","contentType":"application/pdf","size":204800,"visibility":"tenant","createdAt":"2024-01-01T00:00:00Z"}]}`,
				},
				{
					Method:  "POST",
					Path:    "/api/tenant/documents",
					Summary: "Upload document",
					Detail:  "Uploads a private file scoped to the tenant. Send as <code>multipart/form-data</code> with a <code>file</code> field and optional <code>visibility</code> field (<code>tenant</code> or <code>owner</code>). Maximum 50 MB. Files are stored in the configured object store (R2/S3) or MongoDB for local dev.",
					Auth:    "jwt+tenant",
					Response: `{"id":"...","tenantId":"...","ownerId":"...","filename":"tax_return.pdf","contentType":"application/pdf","size":204800,"visibility":"tenant","createdAt":"2024-01-01T00:00:00Z"}`,
				},
				{
					Method:   "GET",
					Path:     "/api/tenant/documents/{id}",
					Summary:  "Get document metadata",
					Detail:   "Returns metadata for a single document. Owner-only documents are not accessible by other tenant members.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "id", Type: "objectId", Required: true, Desc: "Document ID"}},
					Response: `{"id":"...","filename":"tax_return.pdf","contentType":"application/pdf","size":204800,"visibility":"tenant"}`,
				},
				{
					Method:   "GET",
					Path:     "/api/tenant/documents/{id}/download",
					Summary:  "Download document",
					Detail:   "Issues a short-lived presigned redirect to the file in the object store (R2/S3). The URL expires after 15 minutes. On the db-fallback provider (local dev) the file is streamed directly. The response is a <code>302 Found</code> redirect — follow it to receive the file.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "id", Type: "objectId", Required: true, Desc: "Document ID"}},
				},
				{
					...
					Detail:   "Deletes the document from storage and the database. Only the uploader or a tenant admin/owner can delete.",
					Auth:     "jwt+tenant",
					Params:   []apiParam{{Name: "id", Type: "objectId", Required: true, Desc: "Document ID"}},
					Response: `{"status":"ok"}`,
				},
